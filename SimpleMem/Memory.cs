...
public class Memory
{
    ...
    /// <returns></returns>
    protected static Process GetProcess(string processName)
    {
        while (true)
        {
            try
    ...
    public int WriteMemory<T>(IntPtr lpBaseAddress, T value) where T : struct
    {
        ...
        unsafe
        {
            ...
            {
                return this.WriteProcessMemory(this.ProcessHandle, lpBaseAddress, bp, buffer.Length);
            }
        }
    }
    ...
    private int WriteMemory(IntPtr lpBaseAddress, byte[] bytes, bool isProtected = false)
    {
        unsafe
        {
            fixed (byte* bp = bytes)
            {
                return this.WriteProcessMemory(this.ProcessHandle, lpBaseAddress, bp, bytes.Length, isProtected);
            }
        }
    }
    ...
    public T ReadMemory<T>(IntPtr lpBaseAddress, bool isProtected = false) where T : struct
    {
        ...
        unsafe
        {
            fixed (byte* bp = buffer)
            {
                this.ReadProcessMemory(this.ProcessHandle, lpBaseAddress, bp, buffer.Length, isProtected);
            }
        }
    ...
    public int ReadMemory(IntPtr lpBaseAddress, ReadOnlySpan<byte> buffer, bool isProtected = false)
    {
        unsafe
        {
            fixed (byte* bp = buffer)
            {
                return this.ReadProcessMemory(this.ProcessHandle, lpBaseAddress, bp, buffer.Length, isProtected);
            }
        }
    ...
    private List<IntPtr> AoBScan(string pattern, bool once = false)
    {
        ...
        while (procMinAddressL < procMaxAddressL)
        {
            ...
            VirtualQueryEx(this.ProcessHandle, procMinAddress, out var memBasicInfo, 48);
                ...
                memBasicInfo.State == MEM_ALLOC.MEM_COMMIT)
            {
                ...
                unsafe
                {
                    fixed (byte* bp = buffer)
                    {
                        ReadProcessMemory(this.ProcessHandle, new IntPtr((long)memBasicInfo.BaseAddress), bp,
                            CHUNK_SZ, out int _);
                    }
                }
    ...
    public string ReadStringFromMlPtr(MultiLevelPtr mlPtr, int size = 255, bool isNullTerminated = true,
        Encoding encoding = null) => this.ReadString(this.ReadAddressFromMlPtr(mlPtr), size, isNullTerminated, encoding);
